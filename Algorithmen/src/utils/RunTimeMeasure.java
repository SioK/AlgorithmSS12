...
public class RunTimeMeasure<T extends Comparable<T>> {
  ...
  public RunTimeMeasure(List<ISorter<T>> sorters, IDataProvider<T> provider, int loops) {
  ...
  public void execute() {
    ...
    for (ISorter<T> sortAlgorithm : sorters) {
      ...
      System.out.println("Duration of Sort from Algorithm " + sortAlgorithm.getClass().getSimpleName() + " " + mean
          + " millis");
    }
  }
}
